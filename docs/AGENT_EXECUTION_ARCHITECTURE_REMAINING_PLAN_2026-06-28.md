...
- Executor、Finalizer、Coordinator、Handler 仍有较大测试缺口（失败路径、竞态场景），NATS golden 尚未建立。
- 如果”禁止跨层 `map[string]interface{}`”按全仓口径验收，Agent 主链之外仍有存量。
- P0 基线已执行并通过（`gofmt`、全仓测试、vet、race、构建），后续变更后需重新验证。
...
|---|---|---|
| P0 基线 | ✅ 已完成 | — |
| P1 主链行为 | 基本完成 | 失败传播、取消竞态、Close 等边界测试 |
| P1 独立执行契约 | 公共契约完成，内部未完全收敛 | 删除迁移后的 Engine 兼容框架 |
| P2 旧层与全局状态 | 大部分完成 | 清理 provider Registry、Runner 兼容语义 |
| 测试和验收 | 核心路径已有覆盖 | 补 Executor/Finalizer/Coordinator/Handler 测试；NATS golden |
| 文档 | 已更新主体 | 修正旧结论文档 |
