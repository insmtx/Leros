...
但当前仍处于“主链完成、内部实现尚未完全收敛”的阶段：

- Claude、Codex、OpenCode 的装配仍经过 `provider.Registry`
  和 `provider.Engine`。
- `externalcli.Driver.Execute` 仍具有 Runtime 形状，CLI Runtime 只是其外层包装。
- native Runtime 仍通过旧式 Runner 和 `provider.RunRequest` 做二次转换。
- provider 内保留 `RunRequest`、`RunHandle`、`EngineEvent*` 等旧执行框架命名。
- 部分原计划要求的失败、竞态和 wire golden 测试尚未覆盖。
- 最近一次类型别名、Metadata 和 RawPayload 清理后，尚未重新执行最终全仓验证。
- 如果“禁止跨层 `map[string]interface{}`”按全仓口径验收，Agent 主链之外仍有存量。

按最终架构目标评估：

| 范围 | 当前状态 | 剩余重点 |
|---|---|---|
| P0 主链行为 | 基本完成 | 失败传播、取消竞态、Close 等边界测试 |
| P1 独立执行契约 | 公共契约完成，内部未完全收敛 | 删除迁移后的 Engine 兼容框架 |
| P2 旧层与全局状态 | 大部分完成 | 清理 provider Registry、Runner 兼容语义 |
| 测试和验收 | 核心路径已有覆盖 | 公共 Runtime suite、golden、race、全仓验证 |
| 文档 | 已更新主体 | 修正“全部完成”等超前结论 |

整体不应再按“全部完成”验收。后续重点不是重写主链，而是收窄并删除最后一套重复执行抽象。
...
最近一次完整 `go test ./...` 通过发生在最后一轮类型别名、Metadata 和 RawPayload
清理之前。因此，继续删除兼容层前必须先重新建立基线。

### 任务

1. 对当前所有已修改 Go 文件执行 `gofmt`。
2. 执行全仓测试并保存失败分类：
...
- 最新工作区快照的全仓测试通过。
- Coordinator、assistant 和 event sink 无竞态报告。
- 主服务可以构建。
- 测试失败必须区分真实代码错误和环境监听、缓存、外部服务限制，不能通过跳过核心测试获得绿色结果。
...
- 重新运行最终基线。
- 先补 Executor、Coordinator、Handler 和 Finalizer 缺失测试。
