...
本项目遵循 [Uber Go 风格指南](https://github.com/uber-go/guide/blob/master/style.md)。

完整指南请参考：[Uber Go Style Guide](https://github.com/uber-go/guide/blob/master/style.md)

### 导入
- 分三组：标准库、第三方、内部包，组间空行
- 仅在防止命名冲突时使用别名
...
### 命名
- 导出标识符用 `CamelCase`，未导出用 `camelCase`
- 构造函数用 `New` 前缀：`NewAuthService`
- 避免 `Build/Create/Make` 前缀
- 避免 `Get/Set` 前缀，直接访问字段或使用有意义的动词

### 文件命名
- 文件名反映内容，非用途
- ✅ `service.go`, `auth.go`, `router.go`
- ❌ `factory.go`, `builder.go`, `utils.go`
